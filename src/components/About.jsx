...
const About = () => {
  ...
  useEffect(() => {
    const firstInterval = setInterval(() => {
      setNbOfYears((prev) => {
        if (prev < targetYears) {
          return prev + 1; // Incrémente jusqu'à la valeur cible
        } else {
          clearInterval(firstInterval); // Arrête l'intervalle lorsque le nombre est atteint
          return prev;
        }
      });
    }, 1000); // Durée entre chaque incrémentation (50ms)
    const secondInterval = setInterval(() => {
      setProjects((prev) => {
        if (prev < targetProjects) {
          return prev + 1; // Incrémente jusqu'à la valeur cible
        } else {
          clearInterval(secondInterval); // Arrête l'intervalle lorsque le nombre est atteint
          return prev;
        }
      });
    }, 1000); 
    return () => clearInterval(secondInterval); // Nettoie l'intervalle lors du démontage
  }, [targetYears, targetProjects]);
  return (
    <section id='about' className='about-section py-10'>
        <div className='container mx-auto p-4  text-3xl font-bold'>
            <h2 className='text-center mb-8 w-50 uppercase sm:block section_title'>About me</h2>
            ...
            <div className='flex flex-col flex-1 mb-6 md:mb-0'>
              ...
              <div className='flex'>
                <div className='p-8 w-200 h-200 bg-white mr-4 flex flex-col text-center shadow-white border rounded-lg'>
                    ...
                    <span className='text-gray-400 text-sm'>+ {nbOfYears} Years Working</span>
                </div>
            ...
            <div className='text-left text-base sm:text-xl flex-1 w-full md:w-auto border border-red-200 rounded-lg p-3'>
              ...
              <div className='flex items-center mb-4'>
                ...
                <div className='w-full bg-gray-200 rounded-full h-4 ml-4'>
                  <div className='bg-blue-500 h-4 rounded-full animate-progress' style={{ width: '90%' }}></div>
                </div>
              </div>
